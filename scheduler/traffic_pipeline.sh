...
rm -rf "$PLOTS_LOCAL"/{analysis,cleaned_data,models,plots_kmeans,silhouette_scores,traffic_signal_plan,traffic_signal_plan_json} 2>/dev/null || true
rm -rf "$PLOTS_LOCAL"/predict_* 2>/dev/null || true
rm -f "$PLOTS_LOCAL"/prob_congestion_*.png 2>/dev/null || true

mkdir -p "$PLOTS_LOCAL"/{plots_kmeans,silhouette_scores,traffic_signal_plan,traffic_signal_plan_json,analysis,cleaned_data,models}
...
hdfs dfs -get -f "$OUTPUT_PATH"/cleaned_data "$PLOTS_LOCAL"/cleaned_data 2>/dev/null || true
